...
public class MainCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if(args.length == 0) {
            ...
            sender.sendMessage(prefix + "§a clear - 清除消失物品列表");
            sender.sendMessage(prefix + "§6====================");
            return false;
        }
        ...
        if(args.length == 1) {
            ...
            if(args[0].equalsIgnoreCase("clear")) {
                ConfigFile.despawnItems.clear();
                ConfigFile.saveDespawnItems(ItemsProtect.getInstance());
                sender.sendMessage(prefix + ItemsProtect.getInstance().getConfig().getString("messages.clear").replaceAll("&", "§"));
                return false;
            }
        }
        return false;
